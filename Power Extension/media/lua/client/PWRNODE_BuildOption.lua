...
local onNode = function(worldobjects, square, sprite, player)
	-- sprite, northSprite
	local node = IsoObject.new(square,sprite)
	node.renderFloorHelper = true
	node.canBeAlwaysPlaced = true;
	node.containerType = "crate";
...
local onSwitch = function(worldobjects, square, sprite, player)
	-- sprite, northSprite
	local switch = IsoObject.new(square,sprite)
	switch.renderFloorHelper = true
	switch.canBeAlwaysPlaced = true;
	switch.containerType = "crate";
...
local buildNodeMenu = function(subMenu, player,square,worldObjects)
    ...
    local nodeOption = subMenu:addOption("Node", worldObjects, onNode, square, nSprite, player);
    local switchOption = subMenu:addOption("Node", worldObjects, onSwitch, square, sSprite, player);
    local nToolTip,sToolTip = canbuild(2,6,0,0,5,25,player,nodeOption),canbuild(2,4,2,2,25,30,player,nodeOption)
    nToolTip:setName("Generator Switch");
    nToolTip.description = "Extends Generator power through a Switch" .. nToolTip.description;
    nToolTip:setTexture(nSprite.sprite);
    sToolTip:setName("Generator Switch");
    sToolTip.description = "Connect to generator to extend power" .. sToolTip.description;
    sToolTip:setTexture(nSprite.sprite);
...
Events.OnFillWorldObjectContextMenu.Add(function(playerNum, context, worldObjects, test)
    ...
    local player = getSpecificPlayer(playerNum)
        ...
        if square then
            local nodeOption = context:addOption("Node Building", worldObjects, nil);
